<script lang="ts">
	...
	import { HIGHLIGHT_MOCK, EVENTS_MOCK } from '../../lib/mock.js';
	...
	const { isOffline } = get(appConfig);

	const coverURL = (url) => (isOffline ? url : `http://localhost:1337${url}`);

	if (isOffline) {
		highlights = HIGHLIGHT_MOCK;
		events = EVENTS_MOCK;
	}
	...
	function moreEvents() {
		...
		if (isOffline) {
			const updatedEvents = [...events, ...EVENTS_MOCK];
